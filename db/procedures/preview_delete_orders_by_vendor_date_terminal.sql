...
DROP PROCEDURE IF EXISTS preview_delete_orders_by_vendor_date_terminal$$

CREATE PROCEDURE preview_delete_orders_by_vendor_date_terminal(
    IN p_vendors_id INT,
...
main_proc: BEGIN
    ...
    -- Preview order items that would be deleted
    SELECT COUNT(*) INTO v_total_items_count
    FROM orderItems oi
    JOIN orders o ON oi.orders_id = o.id
    WHERE o.vendors_id = p_vendors_id 
    AND o.lastMod >= p_lastmod_from
    AND (p_terminal_id IS NULL OR o.terminals_id = p_terminal_id);
    
    IF v_total_items_count > 0 THEN
        SELECT 'ORDER ITEMS TO DELETE' as info_type, 
               oi.id, oi.orders_id, oi.name, oi.quantity, oi.price
        FROM orderItems oi
        JOIN orders o ON oi.orders_id = o.id
        WHERE o.vendors_id = p_vendors_id 
        AND o.lastMod >= p_lastmod_from
        AND (p_terminal_id IS NULL OR o.terminals_id = p_terminal_id)
        ORDER BY oi.orders_id, oi.id;
    ELSE
        SELECT 'ORDER ITEMS TO DELETE' as info_type, 
               'No order items found matching criteria' as message;
    END IF;
    
    -- Preview order payments that would be deleted
    SELECT COUNT(*) INTO v_total_payments_count
    FROM ordersPayments op
    JOIN orders o ON op.orderReference = o.id
    WHERE o.vendors_id = p_vendors_id 
    AND o.lastMod >= p_lastmod_from
    AND (p_terminal_id IS NULL OR o.terminals_id = p_terminal_id);
    ...
    IF v_total_payments_count > 0 THEN
        SELECT 'ORDER PAYMENTS TO DELETE' as info_type, 
               op.id, op.orderReference, op.paymentMethod, op.amount
        FROM ordersPayments op
        JOIN orders o ON op.orderReference = o.id
        WHERE o.vendors_id = p_vendors_id 
        AND o.lastMod >= p_lastmod_from
        AND (p_terminal_id IS NULL OR o.terminals_id = p_terminal_id)
        ORDER BY op.orderReference, op.id;
    ELSE
        SELECT 'ORDER PAYMENTS TO DELETE' as info_type, 
