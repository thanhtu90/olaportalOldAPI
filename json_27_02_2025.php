...
for ($i = 0; $i < count($orders_json); $i++) {
  ...
  if ($stmt->rowCount() != 0) { //should be zero
    ...
    $stmt = $pdo->prepare("insert into orders set uuid = ?, agents_id = ?, vendors_id = ?, terminals_id = ?, orderReference = ?, subTotal = ?, tax = ?, total = ?, notes = ?, lastMod = ?, employee_id = ?, orderDate = ?, delivery_fee = ?, delivery_type = ?, employee_pin = ?, onlineorder_id = ?, onlinetrans_id = ?, orderName = ?, status = ?, store_uuid = ?");
