...
for ($i = 0; $i < count($orders_json); $i++) {
  ...
  if ($isOnlinePlatform == false) {
    ...
    $stmt = $pdo->prepare("select * from orders where terminals_id = ? and uuid = ?");
    $stmt->execute([$terminals_id, $online_platform_order_uuid]);
  }
  if ($stmt->rowCount() != 0) { //should be zero
    ...
    if ($isOnlinePlatform == true) {
      ...
      error_log("Updated online platform order with uuid " . $online_platform_order_uuid . " to orderReference " . $orderReference . " - Rows affected: " . $stmt_update_online_platform_order->rowCount());
    }
    ...
    for ($j = 0; $j < count($items_json); $j++) {
      if (
        $hasInventory ||
        (!$hasInventory && $payments[$i]->{"orderReference"} == strval($items_json[$j]->{"orderItem"}->{"orderReference"}))
      ) {
        ...
        $stmt = $pdo->prepare("insert into orderItems set 
          ...
          itemDiscount = ?");
        if ($isOnlinePlatform == true) {
          $online_platform_order_group_name = '';
          // query table online_order_groups where id = $col["group"]
          ...
          $online_platform_order_group_name = $row_online_order_groups["name"];
