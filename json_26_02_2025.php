...
function call_broadcast_order_paid($orderUuid)
{
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $url . "broadcast/order-paid/" . $orderUuid);
  curl_setopt($ch, CURLOPT_POST, 1);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
for ($i = 0; $i < count($orders_json); $i++) {
  ...
  if ($stmt->rowCount() != 0) { //should be zero
    ...
    $stmt = $pdo->prepare("insert into orders set uuid = ?, agents_id = ?, vendors_id = ?, terminals_id = ?, orderReference = ?, subTotal = ?, tax = ?, total = ?, notes = ?, lastMod = ?, employee_id = ?, orderDate = ?, delivery_fee = ?, delivery_type = ?, employee_pin = ?, onlineorder_id = ?, onlinetrans_id = ?, orderName = ?, status = ?");
