...
$stmt = $pdo->prepare("SELECT *, count(qty) as qty, price*qty as amt FROM `orderItems`, `ordersPayments` where orderItems.orders_id = ordersPayments.orderReference and items_id != '0' " . $where . " and ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "' group by `description` order by qty desc");
if ( $_REQUEST["type"] == "site" ) {
  $stmt->execute([ ]);
} else {
...
while ( $row = $stmt->fetch() ) {
    ...
    $entry["name"] = $row["description"] . "(" . $row["qty"] . ")";
    $entry["value"] = (int)$row["qty"];
    $entry["amount"] = (float)$row["amt"];
    array_push( $res["count_items"], $entry );
}
/*
$stmt = $pdo->prepare("SELECT *, count(qty)*price as amt FROM `orderItems`, `ordersPayments` where orderItems.orders_id = ordersPayments.orderReference and items_id != '0' " . $where . " and ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "' group by `description` order by amt desc");
$stmt->execute([ $_REQUEST["id"] ]);
$res["max_amount"] = 0;
while ( $row = $stmt->fetch() ) {
    if ( $row["amt"] > $res["max_amount"] ){ $res["max_amount"] = $row["amt"]; }
    $entry["name"] = $row["description"] . "($" . number_format($row["amt"],2) . ")";
