...
if (php_sapi_name() === 'cli') {
    ...
    $options = getopt("", ["vendors_id:", "vendors_ids:", "from-date:", "from-jsonolapay-lastmod:", "to-jsonolapay-lastmod:", "debug"]);
    $vendors_id_filter = $options['vendors_id'] ?? null;
    $vendors_ids_filter = $options['vendors_ids'] ?? null;
    ...
    $vendors_ids_filter = $_GET['vendors_ids'] ?? null;
...
// Merge vendors_id and vendors_ids if both are provided
if ($vendors_id_filter && $vendors_ids_filter) {
    $vendors_id_filter = $vendors_id_filter . ',' . $vendors_ids_filter;
} elseif ($vendors_ids_filter) {
    $vendors_id_filter = $vendors_ids_filter;
}
...
if (!$vendors_id_filter) {
    ...
    $error_message = "Please provide a vendors_id or vendors_ids parameter.\n";
    if (php_sapi_name() === 'cli') {
        $error_message .= "Usage: php reconcile_OlaPay.php --vendors_id=ID [or --vendors_ids=ID1,ID2,...] [--from-date=YYYY-MM-DD] [--from-jsonolapay-lastmod=TIMESTAMP|DATE] [--to-jsonolapay-lastmod=TIMESTAMP|DATE] [--debug]\n";
    } else {
        $error_message .= "Example: ?vendors_id=123&from-date=2024-01-15&debug=true or ?vendors_ids=123,456&from-date=2024-01-15&debug=true";
    }
    die($error_message);
}
