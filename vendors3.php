...
$stmt = $pdo->prepare("select * from accounts where role = 'vendor' or role = 'agent' order by id desc");
$stmt->execute([ ]);
while ( $row = $stmt->fetch() ) {
    ...
    $entry["onboarding_status"] = $row["onboarding_status"];
    $entry["reward_status"] = $row["reward_status"];
