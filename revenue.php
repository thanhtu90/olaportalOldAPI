...
switch ( $_REQUEST["type"] ) {
    ...
    case "terminal":
        ...
        break;
    case "merchant":
	$stmt = $pdo->prepare("select orders.id, sum(ordersPayments.total) as total, max(ordersPayments.lastmod) as lastMod, max(ordersPayments.id) as paymentId, sum(discount*qty) as discount, max(ordersPayments.refNumber) as refNumber from ordersPayments left join orders on ordersPayments.orderReference = orders.id left join orderItems on orderItems.orders_id = orders.id where ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "' and ordersPayments.vendors_id = ? group by orders.id order by max(ordersPayments.lastMod) desc");
