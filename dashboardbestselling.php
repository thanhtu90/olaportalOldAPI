...
$stmt = $pdo->prepare("SELECT description, sum(qty) as qty, sum(price*qty) as amt, price FROM `orderItems`, `ordersPayments` where orderItems.orders_id = ordersPayments.orderReference and items_id = '0' " . $where . " and ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "' group by `description` order by amt desc");
if ( $_REQUEST["type"] == "site" ) {
  $stmt->execute([ ]);
} else {
...
/*
$stmt = $pdo->prepare("SELECT *, sum(price*qty) as amt FROM `orderItems`, `ordersPayments` where orderItems.orders_id = ordersPayments.orderReference and items_id = '0' " . $where . " and ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "' group by `description` order by amt desc");
$stmt->execute([ $_REQUEST["id"] ]);
$res["max_amount"] = 0;
while ( $row = $stmt->fetch() ) {
