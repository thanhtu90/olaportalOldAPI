...
if ( $_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $stmt = $pdo->prepare("SELECT * FROM `orders` WHERE `terminals_id` = ? and `orderReference` = ? and `lastMod` = ?");
    $stmt->execute([ $row["id"], $_REQUEST["orderRef"], $_REQUEST["lastMod"] ]);
    $res = array();
...
if ( $_SERVER["REQUEST_METHOD"] == "PATCH") {
    ...
    #check order availability
    $stmt = $pdo->prepare("SELECT * FROM `orders` WHERE `terminals_id` = ? and `orderReference` = ? and `lastMod` = ?");
    $stmt->execute([ $row["id"], $params["orderRef"], $params["lastMod"] ]);
    $res = array();
    ...
    if ( $stmt->rowCount() == 0 ) {
        ...
        #$row = $stmt->fetch(); 
        $stmt = $pdo->prepare("update `orders` set status = ? WHERE `terminals_id` = ? and `orderReference` = ? and `lastMod` = ?");
	$stmt->execute([ $params["newStat"], $row["id"], $params["orderRef"], $params["lastMod"] ]);
        http_response_code("200");
        $entry = array();
	$entry["orderStat"] = $params["newStat"];
