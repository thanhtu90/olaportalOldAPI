...
$res = array();
#$stmt = $pdo->prepare("select *, ordersPayments.id as id, sum(discount*qty) as discount from ordersPayments left join orders on ordersPayments.orderReference = orders.id left join orderItems on orderItems.orders_id = orders.id where 1 " . $where . " group by orders.id order by ordersPayments.lastMod desc");

$stmt = $pdo->prepare("select *, ordersPayments.total, ordersPayments.lastmod as lastMod, ordersPayments.id as id, sum(discount*qty) as discount, count(distinct ordersPayments.id) as cnt from ordersPayments left join orders on ordersPayments.orderReference = orders.id left join orderItems on orderItems.orders_id = orders.id where ordersPayments.lastMod > '" . $starttime . "' and ordersPayments.lastMod < '" . $endtime . "'" . $where . " group by orders.id order by ordersPayments.lastMod desc");
